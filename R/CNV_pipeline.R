...
#' @param ref_name name of the reference column vector;
...
#' @export
calculate_CNV <- function(genes_by_cells, reference=NULL, ref_name="reference", mc.cores=2, wnd_size=100, 
                          min_genes=1000, min_cells=100, expr_lim=c(-3, 3), scale_cells=TRUE, na.rm=FALSE, 
                          center_genes=FALSE, overwrite = TRUE) {
    ...
    cnv_clustering <- cluster_by_features(ans, cnv=TRUE)
    cnv_res_ref <- apply_CNV_reference(cnv = ans, cnv_clustering = cnv_clustering, ref_name="reference")
    
    #######################################################

    
    if(length(scMuffinList$CNV)==0 | overwrite){
        scMuffinList$CNV <- as.data.frame(ans)
        scMuffinList$partitions <- 
    }else{
        scMuffinList$CNV <- merge(scMuffinList$CNV, as.data.frame(ans), by=0, all=T, sort=F))
        rownames(scMuffinList$CNV) <- scMuffinList$CNV$Row.names
        scMuffinList$gene_set_scoring$Row.names <- NULL
    }
    return(scMuffinList)
