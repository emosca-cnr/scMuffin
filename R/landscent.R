...
landscent <- function(genes_by_cells_matrix, ppi=NULL, reduceMethod = "PCA", clusterMethod = "dbscan", mc.cores=2){
	...
	score <- data.frame(dpt_raw=dpt$dpt, dpt=-dpt$dpt + max(dpt$dpt), SR=Integration.l$SR, PS=Integration.l$potencyState, stringsAsFactors = F, row.names = rownames(Integration.l$DM@eigenvectors))
