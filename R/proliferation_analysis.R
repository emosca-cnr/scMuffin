...
proliferation_analysis <- function(genes_by_cells, mc.cores=2, nbins=25, nmark_min = 5, ncells_min = 5, k=100, kmin=50, score_type=c("relative", "mean"), null_model=TRUE, mean_scale=TRUE){
	...
	tirosh <- calculate_gs_scores(genes_by_cells, gs_list=SIG_Tirosh, mc.cores=mc.cores, nbins=nbins, nmark_min = nmark_min, ncells_min = ncells_min, k=k, kmin=kmin, score_type=score_type, null_model=null_model)
	...
	names(ans) <- colnames(tirosh$gss_by_cells)
