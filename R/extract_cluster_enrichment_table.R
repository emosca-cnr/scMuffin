...
#' @param type "CSEA" or "ORA"
#' @param quantity possible values are as follows. CSEA: es, enrichment score; p_val empirical p-value; adj_p_val, FDR (BH); nes normalized enrichment score; FDRq empirical FDR. ORA: wbd (white balls drawn), number of cells with the value of interest in the cluster; exp expected number of cells with the value of interest in the cluster; er, enrichment ratio; p, hypergeometric test p-value; p_adj, BH FDR
#' @param feature_id name of the features that will be considered. Must be any of `colnames(scMuffinList$cluster_data$[[parition_id]]$CSEA)` or `names(scMuffinList$cluster_data$[[parition_id]]$ORA)`
...
extract_cluster_enrichment_table <- function(scMuffinList, partition_id=NULL, type=NULL, quantity=NULL, feature_id=NULL){
  ...
  if(type == "CSEA" & !is.null(scMuffinList$cluster_data[[partition_id]]$CSEA)){
    ...
    ans <- scMuffinList$cluster_data[[partition_id]]$CSEA
