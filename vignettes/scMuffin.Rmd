...
```
where seu_obj_1$seurat_clusters is a named vector that contains cells clusters and whose names are cell labels. The element `partitions` of the scMuffinList will contain cell clusters:
```{r, echo=TRUE}
...
```{r, include=TRUE, eval=FALSE}
plot_umap_colored_features(scMuffinList_demo, Seu_obj = seu_obj_1, feature_name = "gene_set_scoring")
```
...
```{r, include=TRUE, eval=FALSE}
scMuffinList_demo <- calculate_gs_scores_in_clusters(scMuffinList_demo, partition_id = "global_expr")
```
```{r, include=TRUE, eval=TRUE}
...
```{r, include=TRUE, eval=FALSE}
plot_heatmap_features_by_clusters(scMuffinList_demo, feature_source = "gss")
```
