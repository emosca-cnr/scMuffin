...
```{r, include=TRUE, eval=FALSE}
feat_obj <- create_features(cell_id = colnames(res_sig$SIG_CSEA$signatures_by_cells), values = t(res_sig$SIG_CSEA$signatures_by_cells))
feat_obj <- add_features(feat_obj, cell_id = names(res_chrom_state), values = cbind(cs=res_chrom_state))
```
...
```{r, include=TRUE, eval=FALSE}
cl_enrich <- assess_cluster_enrichment()
```
